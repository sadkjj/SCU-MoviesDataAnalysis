...
const username = ref('')
const password = ref('')
const isAdmin = computed(() => currentUser.value?.role_type === 2)
...
const handleLogin = async () => {
  try {
    ...
    if (response.data.success) {
      ...
      const { user_id, role_type } = response.data
      currentUser.value = { user_id, role_type }
      localStorage.setItem('currentUser', JSON.stringify(currentUser.value))
      router.push(role_type === 2 ? '/admin' : '/user')
    } else {
...
onMounted(() => {
  // 初始化时检查 currentUser 是否为管理员
  if (currentUser.value?.role_type === 2) {
    isAdmin.value = true
  }
})
</script>
